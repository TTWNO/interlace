...
TEST_CASE("Test that file contents are recieved properly when in a list", "[get_file_contents_as_vector_lines]"){
	...
	REQUIRE(get_file_contents_as_vector_lines("txt/hello.txt") == hello_lines);
	REQUIRE(get_file_contents_as_vector_lines("txt/world.txt") == world_lines);
}
...
TEST_CASE("Test that the interlace_vector function works", "[interlace_vecto]"){
	...
	REQUIRE(interlace_vectors({get_file_contents_as_vector_lines("txt/hello.txt"), get_file_contents_as_vector_lines("txt/world.txt")}) == hello_world_vector);
	REQUIRE(interlace_vectors({get_file_contents_as_vector_lines("txt/hello_world.txt"), get_file_contents_as_vector_lines("txt/short_file_test.txt")}) == hw_short_vector);
}

TEST_CASE("Test that the interlace function works with default paramaters, 2 files", "[interlace]"){
    REQUIRE(interlace({"txt/hello_world.txt", "txt/short_file_test.txt"}) == get_file_contents("txt/hw_short.txt"));
